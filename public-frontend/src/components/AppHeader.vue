...
<template>
  ...
  <header class="sticky top-0 z-50 border-b border-black/5 bg-amber-50 shadow-sm">
    <nav class="container-chrome flex items-center justify-between gap-4 py-3">
      <router-link to="/" class="flex min-w-0 shrink items-center gap-2 sm:gap-3 xl:gap-4">
        <img v-if="!logoFailed" :src="staticAsset('images/logo.png')" alt="St Mark's College Namagoma crest" class="h-10 w-10 shrink-0 rounded-full border-2 border-brand-navy bg-white object-contain shadow-sm sm:h-14 sm:w-14 xl:h-16 xl:w-16" @error="logoFailed = true">
        <span v-else class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border-2 border-brand-navy bg-white font-display text-sm font-extrabold text-brand-navy shadow-sm sm:h-14 sm:w-14 sm:text-lg xl:h-16 xl:w-16 xl:text-xl">SM</span>
        <span class="min-w-0 leading-tight">
          <span class="block truncate font-display text-sm font-extrabold text-brand-navy sm:text-base xl:text-lg">St Mark's College Namagoma</span>
          <span class="hidden truncate text-xs italic text-brand-navy/70 sm:block xl:text-sm">The Higher Achiever's College</span>
        </span>
      </router-link>

      <ul class="hidden items-center gap-1 xl:flex xl:gap-2">
        <li v-for="item in menu" :key="item.label" class="group relative">
          <router-link
            v-if="item.to"
            :to="item.to"
            class="flex items-center gap-1 rounded-md px-3 py-2 text-sm font-semibold text-brand-navy transition group-hover:bg-brand-gold/20 xl:px-4 xl:py-2.5 xl:text-base"
          >
            {{ item.label }}
          ...
          <ul v-if="item.children" class="invisible absolute left-0 top-full mt-1 w-60 rounded-xl bg-amber-50 p-2 opacity-0 shadow-card ring-1 ring-black/5 transition group-hover:visible group-hover:opacity-100">
